sap.ui.define([
	...
	return BaseObject.extend("enap.f2.ZHR_SOL_VAC.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias enap.f2.ZHR_SOL_VAC.model.GroupSortState
		 */
		...
		/**
		 * Groups by FleCo, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "FleCo") {
				...
				this._oViewModel.setProperty("/sortBy", "FleCo");

				aSorters.push(
					new Sorter("FleCo", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Begda");
			}

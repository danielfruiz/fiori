		...
		onTheMasterPage: {
			...
			actions: {
				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor(this.createAWaitForAnEntitySet({
						entitySet: "solicitudVacacionesSet",
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Idnum") === oObject.Idnum;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Idnum;
									} else {
										sCurrentId = aItemsNotInTheList[0].Idnum;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Idnum"),
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "solicitudVacacionesSet (0)"
						}),
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'solicitudVacacionesSet (0)'");
						},
				...
				theListShouldBeSortedAscendingOnUnitNumber: function () {
					return this.theListShouldBeSortedAscendingOnField("FleCo");
				},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all solicitudVacacionesSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "solicitudVacacionesSet (" + iExpectedLength + ")"
								}),
								success: function () {
